...
def get_all_attributes(p_userId,p_gameId):
    ...
    if response.get("Item",no_item) == no_item:
        return no_item
...
if __name__ == '__main__':
    try:
        item = get_all_attributes(3,"NTD-001")
        if (item == no_item):
            print(no_item)
        else:
            printScores(item)
        item = get_all_attributes_consistent_read(3,"NTD-001")
        if (item == no_item):
            print(no_item)
        else:
            printScores(item)
        item = get_selected_attributes(3,"NTD-001")
        if (item == no_item):
            print(no_item)
        else:
            printScores(item)
